...
internal sealed class ModuleLoader : IDisposable
{
    ...
    public void Load(IDalamudPluginInterface pluginInterface)
    {
        ...
        try
        {
            ...
            InitializeModule(new global::DDuck.Plugin(), pluginInterface);
#else
            var folder = Path.Combine(pluginInterface.GetPluginConfigDirectory(), "tasks");
            var path = Path.Combine(folder, "DDuck.Access.dll");
            if (!File.Exists(path))
            {
            ...
            var privateJsonPath = Path.Combine(folder, "DDuck.json");
            XA.Access.PrivateManifest.Validate(bytes, File.Exists(privateJsonPath) ? XA.Access.PrivateManifest.ReadFile(privateJsonPath) : null, "DDuck");
    ...
    private sealed class ModuleContext : AssemblyLoadContext
    {
        ...
        public ModuleContext() : base("DDuck.Access." + Guid.NewGuid().ToString("N"), true) { }
